...
/*
 * Some general info about nv50 architecture
 *
 * Registers:
 * - normal registers: $r0 - $r127. Each is 32-bit. You need to declare how
 *   many of them you use. Memory stores from unallocated regs are *ignored*,
 *   other reads return 0. Used for outputs in FP. In some, but not all,
 *   16-bit instructions, you can use low and high halves of $r registers,
 *   like $r0h and $r0l. 64-bit insns use register pairs, like $r2d [== $r2 as
 *   low part, $r3 as high part]. 128-bit insns use register quads, like $r8q
 *   [== $r8 - $r11]. Seems like pairs need to start at even register index,
 *   quads at divisible by 4.
 * - address registers: $a0-$a7. Used for addressing shared, local and const
 *   memory. $a1-$a4 are for normal use, $a0 is special and always set to 0.
 *   Known as $ofs1-$ofs7 in decuda. Each is 16-bit. $a5-$a6 don't work, $a7
 *   seems to work, but isn't used by ptxas for some reason.
 * - condition registers: $c0-$c3. Each has 4 btis: Z, S, C, O. Set according
 *   to results of an instruction if requested. Used for conditional execution.
 * - output registers: $o0 - $o127. Write-only, normally useful only in VP
 *   and GP, as outputs. Each is 32-bit. $o63h and $o127 are bit buckets,
 *   writes to them have no effect [this makes $o127 unusable for output, but
 *   means that you can use $o63h as bit bucket without disturbing $o63].
 *   Trying to write to either low or high half of an $o register just
 *   duplicates the value across *both* low and high parts, so is probably
 *   useless.
 *
 ...
 * - varying memory: v[...]. Exists only in FP and is used for interpolated
 *   inputs from VP. Something weird happens here.
 * - global memory, 16 times: g0[...]-g15[...]. Shared between everything. Can
 *   do 64-bit and 128-bit accesses. Uses 32-bit addressing, probably limited
 *   only by VRAM size. XXX usable outside CP? probably not.
 * - constant memory, 16 times: c0[...]-c15[...]. Shared between everything,
 *   read-only. Contains constants used in program or uniform inputs. Usable
 *   in everything. Each of them can be up to 64kB long.
 *
 * Bits in $c registers:
 *  0: Z, set if result 0 or NAN
 *  1: S, set if result is negative or NAN
 *  2: C, set if carry out of addition/substraction [carry, not borrow, for sub]
 *  3: O, signed overflow flag. set if high bit of integer computation is not
 *     the sign of the "true" result.
 *
 * For integer->integer cvts, flags S and Z are set according to the register
 * that is destination of the operand, C and O are 0. Note that with s8/u8
 * destination, flags are set according to the *whole* 16- or 32-bit
 * destination register, not just lowest 8 bits.
 *
 * NV50 code can be used in 4 flavors:
 *
 * - VP [Vertex Program]: Inputs are in a[], outputs are in $o registers.
 * - GP [Geometry Program]: XXX Unexplored
 * - FP [Fragment Program]: Inputs are in v[], outputs are in $r registers.
 *   Trying to access s[]/a[] crashes.
 * - CP [Compute Program]: Has controllable everything, has proper s[].
 *
