...
int main(int argc, char **argv) {
	...
	const struct disvariant *ctx_var_nv40 = ed_getvariant(ctx_isa, "nv40");
	const struct disvariant *ctx_var_nv50 = ed_getvariant(ctx_isa, "nv50");
			...
			for (cci = 0; cci < cctxnum; cci++) {
				...
				if (cc->bar0 && addr >= cc->bar0 && addr < cc->bar0+cc->bar0l) {
					...
					if (cc->hwsqip && addr != cc->hwsqnext) {
						envydis(ed_getisa("hwsq"), stdout, cc->hwsq, 0, cc->hwsqnext & 0x3fc, -1, -1, 0, 0, 0, discolors);
						cc->hwsqip = 0;
					}
					if (addr == 0 && !cc->chdone) {
						char chname[5];
						...
						envydis(ctx_isa, stdout, param, cc->ctxpos, 4, (cc->arch == 5 ? ctx_var_nv50 : ctx_var_nv40)->feats, -1, 0, 0, 0, discolors);
						cc->ctxpos++;
						free(ai->name);
						free(ai);
						skip = 1;
