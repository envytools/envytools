	...
	switch (elem->type) {
		case RNN_ETYPE_REG:
			for(i = 0; i < elem->length; ++i) {
					if(elem->length >= 2)
						printf("d %s %08Lx %s(%i)", prefix, base + elem->offset + i * elem->stride, elem->fullname, i);
					else
						printf("d %s %08Lx %s", prefix, base + elem->offset, elem->fullname);
		...
		case RNN_ETYPE_ARRAY:
			for (i = 0; i < elem->length; i++)
				for (j = 0; j < elem->subelemsnum; j++)
					expand(prefix, elem->subelems[j], base + elem->offset, width);
			break;
	}
}
